...
func TestBalancesCreateConcurrentCreatePreservesReplayForAllKeys(t *testing.T) {
	...
	// different keys and responses both pass the existence check before either
	// writes. Shared fields become last-write-wins, but each per-key response
	// snapshot survives the additive metadata merge.
	var (
		getCalls int
		addCalls int
	...
	create := func(idempotencyKey string, request wallet.CreateBalance) *wallet.Balance {
		req := newRequest(t, http.MethodPost, "/wallets/"+walletID+"/balances", request)
		req.Header.Set("Idempotency-Key", idempotencyKey)
		rec := httptest.NewRecorder()
		testEnv.Router().ServeHTTP(rec, req)
	...
	expiresAtB := time.Now().Add(2 * time.Hour)
	requestA := wallet.CreateBalance{Name: balanceName, Priority: 1, ExpiresAt: &expiresAtA}
	requestB := wallet.CreateBalance{Name: balanceName, Priority: 2, ExpiresAt: &expiresAtB}

	// Two racing first-time creates with different keys both succeed and both
	// write different shared priority/expiration values.
	firstA := create("key-A", requestA)
	firstB := create("key-B", requestB)
	require.Equal(t, 2, addCalls)

	// Both keys replay their own original result, even though the live account's
	// shared fields now contain the later key-B values.
	require.Equal(t, firstA, create("key-A", requestA))
	require.Equal(t, firstB, create("key-B", requestB))
